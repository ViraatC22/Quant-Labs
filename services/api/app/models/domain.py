...
class JournalEntry(TimestampMixin, Base):
    ...
    journal_metadata: Mapped[dict] = mapped_column("metadata", JSONB, default=dict, nullable=False)
...
class Trade(TimestampMixin, Base):
    ...
    setup_id: Mapped[UUID | None] = mapped_column(PG_UUID(as_uuid=True))
    timeframe: Mapped[str | None] = mapped_column(String(32))
    session: Mapped[str | None] = mapped_column(String(80))
    market_regime_id: Mapped[UUID | None] = mapped_column(PG_UUID(as_uuid=True))
    ...
    trade_metadata: Mapped[dict] = mapped_column("metadata", JSONB, default=dict, nullable=False)
...
class KgEdge(Base):
    ...
    from_node_id: Mapped[UUID] = mapped_column(
        PG_UUID(as_uuid=True), ForeignKey("kg_nodes.id"), nullable=False, index=True
    )
